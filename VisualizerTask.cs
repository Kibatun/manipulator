...
namespace Manipulation
{
    public static class VisualizerTask
    {
        ...
        public static void DrawManipulator(Graphics graphics, PointF shoulderPos)
        {
            ...
            for (int i = 0; i <= joints.Length; i++)
                graphics.FillEllipse(JointBrush, points[i].X - JOINT_RADIUS, points[i].Y - JOINT_RADIUS,
                    2 * JOINT_RADIUS, 2 * JOINT_RADIUS);
        }

        private static void DrawReachableZone(Graphics graphics, Brush reachableBrush, Brush unreachableBrush,

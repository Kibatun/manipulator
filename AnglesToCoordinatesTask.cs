...
namespace Manipulation
{
 ...
 (float)(Manipulator.UpperArm * Math.Sin(shoulder)));
            var wristPos = new PointF((float)(Manipulator.Forearm * Math.Cos(angle2)) +
 elbowPos.X, (float)(Manipulator.Forearm * Math.Sin(angle2)) + elbowPos.Y);
            var palmEndPos = new PointF((float)(Manipulator.Palm * Math.Cos(angle3)) +
 wristPos.X, (float)(Manipulator.Palm * Math.Sin(angle3)) + wristPos.Y);
            return new PointF[]
            {
                elbowPos,
                wristPos,
    ...
    public class AnglesToCoordinatesTask_Tests
    {
        ...
        [TestCase(Math.PI, Math.PI / 2, 3 * Math.PI / 2, -
